...
class PatientRegistrationView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            db.collection('patients').document(uid).set(doctor_data)
...
class PatientProfileView(APIView):
    ...
    def get(self, request, uid):
        try:
            doctor_ref = db.collection('patients').document(uid)
...
@method_decorator(csrf_exempt, name='dispatch')
class TogglePatientStatusView(APIView):
    permission_classes = [AllowAny]
    ...
    def post(self, request, uid):
        try:
            ...
            return Response({
                'message': f'Patient status changed to {"active" if new_status else "inactive"}',
                'is_active': new_status
            }, status=status.HTTP_200_OK)
...
class PatientListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            patients_ref = db.collection('patients')
            if active_only:
                docs = patients_ref.where('is_active', '==', True).stream()
            else:
...
@method_decorator(csrf_exempt, name='dispatch')
class CheckPatientAvailabilityView(APIView):
    permission_classes = [AllowAny]
        ...
        try:
            ...
            if not doctor_data.get('is_active', True):
                return Response({
                    ...
                    'day': day,
                    'is_available': False,
                    'message': 'Patient is currently offline'
                }, status=status.HTTP_200_OK)
...
class BookAppointmentView(APIView):
    ...
    def post(self, request, doctor_uid):
        ...
        try:
            doctor_ref = db.collection('patients').document(doctor_uid)
            doctor_doc = doctor_ref.get()
            ...
            if not doctor_data.get('is_active', True):
                return Response({
                    'error': 'Patient is currently offline and not accepting appointments'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            if day_index is None:
                return Response({
                    'error': f'Patient has no availability set for {data["day"]}'
                }, status=status.HTTP_400_BAD_REQUEST)
...
class CancelAppointmentView(APIView):
    ...
    def post(self, request, booking_id):
        try:
            ...
            doctor_ref = db.collection('patients').document(appointment_data['doctor_uid'])
            doctor_doc = doctor_ref.get()
